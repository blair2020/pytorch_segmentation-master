...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None):
        ...
        if config["use_synch_bn"]:  # 同步批归一化，后续再看
        ...
        # CONFIGS
        cfg_trainer = self.config['trainer']  # config.json里的
        self.epochs = cfg_trainer['epochs']  # epoch数
        self.save_period = cfg_trainer['save_period']  # 多少epoch保存一次

        # OPTIMIZER
        if self.config['optimizer']['differential_lr']:  # 编码器和解码器不同的学习率
            if isinstance(self.model, torch.nn.DataParallel):  # 加了层module
                trainable_params = [
        ...
        # MONITORING
        # 字典的get方法，dict.get(键,没有该键得到的值)
        self.monitor = cfg_trainer.get('monitor', 'off')  # "max Mean_IoU"
        if self.monitor == 'off':
            self.mnt_mode = 'off'
            self.mnt_best = 0
        else:
        ...
        start_time = datetime.datetime.now().strftime('%m-%d_%H-%M')  # 拿日期时间，后面做文件夹名
        self.checkpoint_dir = os.path.join(cfg_trainer['save_dir'], self.config['name'], start_time)
        helpers.dir_exists(self.checkpoint_dir)  # 应该是检测文件夹存在的，后续再看
        config_save_path = os.path.join(self.checkpoint_dir, 'config.json')
        with open(config_save_path, 'w') as handle:  # 保存json文件
    ...
    def _get_available_devices(self, n_gpu):  # n_gpu为json里设置的
        os.environ["CUDA_VISIBLE_DEVICES"] = '1,2'
        sys_gpu = torch.cuda.device_count()  # 电脑带有的GPU数
        if sys_gpu == 0:  # 如果没有检测到GPU
            self.logger.warning('No GPUs detected, using the CPU')  #
            n_gpu = 0  # GPU数设置为0
        elif n_gpu > sys_gpu:  # 电脑带有的GPU小于json里设置的GPU数
            self.logger.warning(f'Nbr of GPU requested is {n_gpu} but only {sys_gpu} are available')
            n_gpu = sys_gpu  # GPU数设置为电脑带有的GPU数

        device = torch.device('cuda:1' if n_gpu > 0 else 'cpu')  # 指定设备，待会将模型加载到上面
        self.logger.info(f'Detected GPUs: {sys_gpu} Requested: {n_gpu}')
        available_gpus = list(range(n_gpu))
        available_gpus = [i + 1 for i in list(range(n_gpu))]
        # 返回指定设备和可用GPU列表
    ...
    def _resume_checkpoint(self, resume_path):  # 接着训练，后续再看
