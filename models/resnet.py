...
class Bottleneck(nn.Module):
    ...
    expansion = 4  # expansion是残差结构中输出维度是输入维度的多少倍
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True, multi_grid=False,
        ...
        self.inplanes = 128 if deep_base else 64  # 128
        super(ResNet, self).__init__()
        if deep_base:  # 是
            self.conv1 = nn.Sequential(
                nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False),  # 1/2
                norm_layer(64),
                nn.ReLU(inplace=True),
                nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False),  # 不变
                norm_layer(64),
                nn.ReLU(inplace=True),
                nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1, bias=False),  # 不变
            )
        else:
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, norm_layer=None, multi_grid=False):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:  # 128！=64*4
...
def resnet50(pretrained=False, root='./pretrained', **kwargs):  # 用的这个
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    # 因为ResNet一般有4个stack，每一个stack里面都是block的堆叠，所以[3, 4, 6, 3]就是每一个stack里面堆叠block的个数
